...
# Every name resolvable without an import (``Exception``, ``property``, …).
# Used only to suppress false positives in undefined-name detection.
...
def generate_module_stub(
    ...
    *,
    ...
    if typing_imports:
        ...
        head.append(f"from typing import {names}")

    # external imports
    for mod in sorted(extra_imports):
        head.append(_render_import_line(mod, extra_imports[mod]))
    ...
    # Class stubs
    for info in classes:
        body.append(_render_class(info, partial_class_fallback))

    source = "\n".join(head + body) + "\n"

    # Undefined-name fallbacks ─────────────────────────────────────
    # A .pyi *replaces* its module for type checkers, so any name the stub
    # references but neither imports nor defines (a sibling class in the
    # same module, a private helper type, ``NoneType``, …) is an error.
    # The module-level ``__getattr__`` does not help here: it only covers
    # attribute access from *outside*. Bind the leftovers to ``Any``.
    fallbacks = _unresolved_names(source)
    if fallbacks:
        alias_block = [
            "# Names referenced by this stub that are not importable from here",
            "# (module-private helpers, same-module siblings). Typed as Any so",
            "# the stub stays valid.",
        ]
        alias_block.extend(f"{name} = Any" for name in fallbacks)
        alias_block.append("")
        source = "\n".join(head + alias_block + body) + "\n"
...
def _unresolved_names(source: str) -> list[str]:
    """Return names *source* uses in type positions but never binds.

    Returns an empty list when the stub cannot be parsed — the caller is
    then no worse off than before.
    """
    try:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.AnnAssign):
            _collect_type_names(node.annotation, used)
        elif isinstance(node, ast.arg) and node.annotation is not None:
            _collect_type_names(node.annotation, used)
        elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            if node.returns is not None:
                _collect_type_names(node.returns, used)
        elif isinstance(node, ast.ClassDef):
            for base in node.bases:
                _collect_type_names(base, used)
            for kw in node.keywords:
                _collect_type_names(kw.value, used)

    return sorted(used - bound - _BUILTIN_NAMES - _PY_BUILTINS)


def _module_level_bindings(tree: ast.Module) -> set[str]:
    """Names bound at module level in a generated stub."""
    bound: set[str] = set()
...
def _collect_type_names(node: ast.expr, out: set[str]) -> None:
    """Collect identifiers a type expression depends on.

    Recurses through string constants (forward references) but deliberately
    stops at two places where strings are *values*, not types:
    ``Literal["a", "b"]`` subscripts and call arguments such as
    ``Field(description="...")``.
    """
    ...
    if isinstance(node, ast.Constant):
        if isinstance(node.value, str):
            try:
                inner = ast.parse(node.value.strip(), mode="eval")
            except (SyntaxError, ValueError):
                return
            _collect_type_names(inner.body, out)
        return
    if isinstance(node, ast.Subscript):
        _collect_type_names(node.value, out)
        if not _is_literal_ref(node.value):
            _collect_type_names(node.slice, out)
        return
    if isinstance(node, ast.Call):
        _collect_type_names(node.func, out)  # args are values, not types
        return
...
def _is_literal_ref(node: ast.expr) -> bool:
    """Return ``True`` for ``Literal`` / ``typing.Literal`` references."""
    if isinstance(node, ast.Name):
...
def _render_class(info: ClassStubInfo, partial_class_fallback: bool) -> str:
    ...
    for method in info.methods:
        ...
        body_lines.append(f"    {keyword} {method.name}{method.signature}: ...")
